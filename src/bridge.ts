...
export function scanFile(absPath: string): Promise<EncodingScan> {
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', (chunk) => scanner.push(chunk as Buffer))
 ...
 * 临时文件清理由调用方负责。
 */
export async function overwriteInPlace(tempPath: string, absPath: string): Promise<void> {
...
async function detectForRestore(absPath: string, size: number): Promise<DetectedEncoding> {
  if (size <= IN_MEMORY_LIMIT) return detectEncoding(await fsp.readFile(absPath))
  return decideFromScan(await scanFile(absPath))
}
...
async function recodeInPlace(absPath: string, size: number, from: DetectedEncoding, to: DetectedEncoding): Promise<void> {
...
export function isDefinitivelyGone(error: unknown): boolean {
...
export async function grepFiles(opts: GrepOptions): Promise<GrepMatch[]> {
  ...
  if (stat.isFile()) {
    ...
    if (BINARY_EXTENSIONS.has(path.extname(opts.target).toLowerCase())) return []
...
export type ConvertOutcome =
  | { kind: 'converted'; persist: boolean; encoding: DetectedEncoding }
  | { kind: 'already-utf8' }
  | { kind: 'ledger-cleared' }

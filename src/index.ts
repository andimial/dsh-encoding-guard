...
type RestoreOutcome = 'restored' | 'gone' | 'external' | 'failed'
...
export function apply(ctx: Context): void {
  ...
  async function ensureUtf8NoBom(
    absPath: string,
    key: string,
    sessionId: string,
    signal?: AbortSignal,
  ): Promise<'converted' | 'already' | 'skipped'> {
    if (ledger.has(key)) {
      ledger.touch(key)
      return 'already'
    }
    ...
    if (bytes.length === 0 || bytes.length > MAX_SCAN_BYTES) return 'skipped'
  ...
  async function restoreOne(key: string, entry: LedgerEntry): Promise<RestoreOutcome> {
    let bytes: Buffer
    try {
  ...
  async function restoreAll(): Promise<string[]> {
    const report: string[] = []

...
补位工具族（见 ADR 0001）中的只读路径 eb_grep / eb_peek 不写磁盘、不进账本：走"读字节 → 检测 → 解码 → 返回"的内存解码。eb_convert 是例外——它是显式磁盘转换，语义见下段。被拒方案是复用磁盘转换桥（ensureUtf8OnDisk）——一次 grep 扇出会临时改写大量文件，恢复窗口内 git/IDE/同步盘都观察到假状态，违背最小惊讶；代价是与磁盘桥并存两套读路径，可接受，因为 write/edit 语义仍由磁盘桥独占保证。

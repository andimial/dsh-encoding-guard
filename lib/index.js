...
import { IN_MEMORY_LIMIT, ensureUtf8OnDisk, restoreAll, restoreOne, peekFile, grepFiles, convertFile, } from './bridge.js';
...
export function apply(ctx) {
    ...
    ctx.on('tools/execute', async (exec, next) => {
        ...
        if (preliminary.kind === 'pass')
            return next();
        const absPath = await resolveToolPath(rawArgs.file_path, exec);
        if (!absPath)
            return next();

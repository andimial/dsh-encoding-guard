 ...
 *
 * 尺寸分层：≤ IN_MEMORY_LIMIT 走整缓冲路径（行为与旧版一致）；更大走流式路径。
 * 自动桥（read/write/edit 前置转换）超过 MAX_SCAN_BYTES 跳过；eb_convert 显式补位，无上限。
 */
...
import { BOM_BYTES, decideFromScan, bomLength, baseEncoding, detectEncoding, decodeToText, encodeFromText, } from './encoding.js';
...
export function normalizeCrlfToLf(text) {
...
class StreamScanner {
    ...
    gbDecoder = iconv.getDecoder('gb18030');
    push(chunk) {
...
export async function detectEncodingFile(absPath) {
    try {
 ...
 * 临时文件清理由调用方负责。
 */
export async function overwriteInPlace(tempPath, absPath) {
    const bytes = await fsp.readFile(tempPath);
    const backup = await fsp.readFile(absPath);
    try {
        await fsp.writeFile(absPath, bytes);
    }
    catch (error) {
        await fsp.writeFile(absPath, backup).catch(() => { });
        throw error;
    }
}
/**
 ...
 * - 任何失败：删除临时文件、原文件字节不动。
 */
export async function recodeFile(absPath, from, to) {
    const fromBom = bomLength(from);
    const toBom = bomLength(to);
    const tempPath = `${absPath}.ebtmp`;
    try {
        await pipeline(createReadStream(absPath, { highWaterMark: CHUNK_SIZE }), fromBom > 0 ? new StripPrefix(fromBom) : new PassThrough(), iconv.decodeStream(baseEncoding(from)), iconv.encodeStream(baseEncoding(to)), 
        // toBom>0 ⇒ 目标必为 -bom 形态，基础编码不可能是 gb18030（无 BOM 形态）
        toBom > 0 ? new InjectPrefix(BOM_BYTES[baseEncoding(to)]) : new PassThrough(), createWriteStream(tempPath, { highWaterMark: CHUNK_SIZE }));
        try {
            await fsp.rename(tempPath, absPath);
        }
        catch {
            // rename 失败回退：临时内容原地覆盖（外部进程持有句柄场景）
            await overwriteInPlace(tempPath, absPath);
        }
    }
    catch (error) {
...
/** 检测（错误上抛版）：恢复路径用——调用方按错误类型分流 gone/failed。 */
async function detectForRestore(absPath, size) {
    if (size <= IN_MEMORY_LIMIT)
        return detectEncoding(await fsp.readFile(absPath));
    return decideFromScan(await scanFile(absPath));
}
/** 检测某文件并按需转 UTF-8（账本幂等：已记账直接 already）。读取失败返回 undefined（放行/报错）。 */
async function detectForBridge(absPath, size) {
    try {
        return await detectForRestore(absPath, size);
    }
    catch {
        return undefined;
    }
}
/** 按尺寸分层原地转码（磁盘桥 / 恢复 / eb_convert 共用）：小文件整缓冲，大文件流式原子管道。 */
async function recodeInPlace(absPath, size, from, to) {
    if (size <= IN_MEMORY_LIMIT) {
        const text = decodeToText(await fsp.readFile(absPath), from);
        await fsp.writeFile(absPath, encodeFromText(text, to));
    }
    else {
        await recodeFile(absPath, from, to);
    }
}
/**
 ...
 */
export async function ensureUtf8OnDisk(absPath, ledger, sessionId) {
    ...
    if (enc === undefined || enc === 'utf8' || enc === 'empty' || enc === 'binary')
        return 'skipped';
    await recodeInPlace(absPath, size, enc, 'utf8');
...
export async function restoreOne(ledger, key, entry) {
    try {
        const size = (await fsp.stat(entry.path)).size;
        const current = await detectForRestore(entry.path, size);
        if (current !== 'utf8' && current !== 'utf8-bom') {
            // 磁盘已非 UTF-8 状态（外部改动或已被恢复）：视为已了结
            ledger.delete(key);
            return 'external';
        }
        await recodeInPlace(entry.path, size, current, entry.encoding);
        ledger.delete(key);
        return 'restored';
    }
    catch (error) {
        if (isDefinitivelyGone(error)) {
            ledger.delete(key); // 文件已确定不在（删除/路径变目录等）：账本自然清空
            return 'gone';
        }
...
export async function restoreAll(ledger) {
    const report = [];
...
export async function peekFile(absPath, opts = {}) {
...
async function collectFiles(dir, includeRe, out) {
    ...
    for (const entry of entries) {
        ...
        if (!entry.isFile())
            continue;
        if (includeRe !== undefined && !includeRe.test(entry.name))
            continue;
        if (BINARY_EXTENSIONS.has(path.extname(entry.name).toLowerCase()))
            continue;
        try {
            if ((await fsp.stat(full)).size > MAX_SCAN_BYTES)
                continue;
        }
        catch {
            continue;
...
export async function grepFiles(opts) {
    ...
    const files = [];
    if (stat.isFile()) {
        // 与目录遍历一致：二进制扩展名单文件 target 同样跳过（README 承诺的行为）
        if (BINARY_EXTENSIONS.has(path.extname(opts.target).toLowerCase()))
            return [];
...
export async function convertFile(absPath, persist, ledger, sessionId) {
    const key = absPath.toLowerCase();
    let size;
    try {
        size = (await fsp.stat(absPath)).size;
    ...
    const enc = await detectForBridge(absPath, size);
    if (enc === undefined)
        return { kind: 'error', message: `无法读取或检测：${absPath}` };
    if (enc === 'binary')
        return { kind: 'error', message: `检测为二进制，不做文本编码转换：${absPath}` };

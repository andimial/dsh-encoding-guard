...
export function decideFromScan(scan) {
    if (scan.length === 0)
        return 'empty';
    if (scan.length >= 3 && scan.head[0] === 0xEF && scan.head[1] === 0xBB && scan.head[2] === 0xBF)
        return 'utf8-bom';
    if (scan.length >= 2 && scan.head[0] === 0xFF && scan.head[1] === 0xFE)
        return 'utf16le-bom';
    if (scan.length >= 2 && scan.head[0] === 0xFE && scan.head[1] === 0xFF)
        return 'utf16be-bom';
    if (scan.zeroEven + scan.zeroOdd > 0)
        return bareUtf16FromScan(scan) ?? 'binary';
    if (scan.utf8Valid)
        return 'utf8';
    if (scan.gb18030Valid)
        return 'gb18030';
    return 'binary';
}
...
/** 解码用的基础编码名（剥离 -bom 形态）。 */
export function baseEncoding(enc) {
    switch (enc) {
...
export function decodeToText(bytes, enc) {
    switch (enc) {
        case 'utf8-bom':
...
export function encodeFromText(text, enc) {
    switch (enc) {
        case 'utf8-bom':
        case 'utf16le-bom':
        case 'utf16be-bom': {
